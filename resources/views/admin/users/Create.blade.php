    ...
    <form action="{{ route('admin.users.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div>
                ...
                <input type="text" id="nama" name="nama" value="{{ old('nama') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>
            <div>
        ...
        <div class="mb-4">
            ...
            <select id="role" name="role" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" onchange="toggleSiswaFields()" required>
                ...
                <option value="kantin" {{ old('role') == 'kantin' ? 'selected' : '' }}>Kantin</option>
                <option value="siswa" {{ old('role') == 'siswa' ? 'selected' : '' }}>Siswa</option>
                <option value="administrator" {{ old('role') == 'administrator' ? 'selected' : '' }}>Administrator</option>
            </select>
        </div>
        
        <div id="siswa-fields" class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4 {{ old('role') == 'siswa' ? '' : 'hidden' }}">
            <div>
                <label for="nisn" class="block text-gray-700 text-sm font-bold mb-2">NISN (Wajib untuk Siswa):</label>
                <input type="text" id="nisn" name="nisn" value="{{ old('nisn') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div>
                <label for="kelas" class="block text-gray-700 text-sm font-bold mb-2">Kelas (Wajib untuk Siswa):</label>
                <input type="text" id="kelas" name="kelas" value="{{ old('kelas') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
        </div>
