...
@if(request()->has('nisn'))
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-white shadow-md rounded-lg p-6">
                ...
                <div class="space-y-4 max-h-96 overflow-y-auto">
                    ...
                    <div class="flex items-center justify-between p-3 border rounded-lg">
                        ...
                        <input type="number" name="items[{{ $loop->index }}][jumlah]" min="0" max="{{$barang->stok}}" data-id-barang="{{$barang->id_barang}}" data-harga="{{$barang->harga}}" placeholder="0" class="w-24 p-2 border rounded-lg text-center item-jumlah">
                        <input type="hidden" name="items[{{ $loop->index }}][id_barang]" value="{{ $barang->id_barang }}">
                    </div>
                    @empty
            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <div class="space-y-2 text-gray-700">
                    ...
                    <div class="flex justify-between items-center text-lg font-semibold mt-1">
                        <span>Sisa Saldo:</span>
                        <span id="sisa-saldo" class="text-green-600">Rp {{ number_format($siswa->saldo, 0, ',', '.') }}</span>
                    </div>
                </div>
    ...
    @else
    <p class="text-red-500 text-center font-semibold">Siswa dengan NISN "{{ request('nisn') }}" tidak ditemukan.</p>
    @endif
@endif
@endsection
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!saldoAwalEl) return; // Keluar jika elemen tidak ditemukan
    ...
    calculateTotal(); // Hitung saat pertama kali halaman dimuat
});
</script>
@endpush